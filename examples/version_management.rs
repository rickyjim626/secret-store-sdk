...
async fn main() -> Result<()> {
    ...
    let result = client
        .put_secret(namespace, key, "version 1", PutOpts::default())
        .await?;
    println!("Created: {:?}\n", result);
    ...
    let result = client
        .put_secret(namespace, key, "version 2", PutOpts::default())
        .await?;
    println!("Updated: {:?}\n", result);
    ...
    let result = client
        .put_secret(namespace, key, "version 3", PutOpts::default())
        .await?;
    println!("Updated: {:?}\n", result);
    ...
    for version in &versions.versions {
        println!(
            "  - Version {}: created at {} by {}",
            version.version, version.created_at, version.created_by
        );
    }
    println!();
    ...
    let current = client
        .get_secret(namespace, key, Default::default())
        .await?;
    println!(
