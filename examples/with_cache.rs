...
async fn basic_cache_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    client.put_secret(namespace, key, "config-value", PutOpts::default()).await?;
    ...
    let secret1 = client.get_secret(namespace, key, GetOpts::default()).await?;
    let duration1 = start.elapsed();
    println!("  Value: {} (v{})", secret1.value.expose_secret(), secret1.version);
    println!("  Time: {:?}", duration1);

    // Second request - cache hit
    println!("\nSecond request (cache hit):");
    let start = Instant::now();
    let secret2 = client.get_secret(namespace, key, GetOpts::default()).await?;
    let duration2 = start.elapsed();
    println!("  Value: {} (v{})", secret2.value.expose_secret(), secret2.version);
    ...
    println!("  Value: {} (v{})", secret3.value.expose_secret(), secret3.version);
...
async fn conditional_request_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create a secret
    client.put_secret(namespace, key, "initial-value", PutOpts::default()).await?;
    ...
    let last_modified = secret.last_modified.clone().expect("Should have Last-Modified");
    ...
    println!("  Got value (from 304 response): {}", secret2.value.expose_secret());
    
    // Update the secret
    println!("\nUpdating secret...");
    client.put_secret(namespace, key, "updated-value", PutOpts::default()).await?;
...
async fn performance_comparison() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for _ in 0..3 {
        for i in 0..10 {
            let _ = client_no_cache.get_secret(
                namespace, 
                &format!("perf-key-{}", i),
                GetOpts::default()
            ).await?;
    ...
    for _ in 0..3 {
        for i in 0..10 {
            let _ = client_with_cache.get_secret(
                namespace, 
                &format!("perf-key-{}", i),
                GetOpts::default()
            ).await?;
...
async fn cache_management_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..5 {
        ...
        client.put_secret(namespace, &key, &format!("value-{}", i), PutOpts::default()).await?;
    ...
    let _ = client.get_secret(namespace, "cache-key-2", GetOpts::default()).await?;
    ...
    client.put_secret(namespace, "cache-key-3", "new-value-3", PutOpts::default()).await?;
    
    // Fetch should get new value
    let updated = client.get_secret(namespace, "cache-key-3", GetOpts::default()).await?;
    ...
    for i in 0..5 {
        let _ = client.get_secret(namespace, &format!("cache-key-{}", i), GetOpts::default()).await?;
    }
    
    println!("  All entries cleared, new misses: {}", 
...
async fn custom_cache_config_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    client_short.put_secret(namespace, "short-ttl", "value", PutOpts::default()).await?;
    client_short.get_secret(namespace, "short-ttl", GetOpts::default()).await?;
    ...
    client.put_secret(namespace, "ttl-secret", "expires-soon", opts).await?;
    
    let secret = client.get_secret(namespace, "ttl-secret", GetOpts::default()).await?;
