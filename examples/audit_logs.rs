...
use anyhow::Result;
use secret_store_sdk::{Auth, AuditQuery, ClientBuilder};
...
async fn main() -> Result<()> {
    ...
    println!("Showing {} of {} entries\n", audit_logs.entries.len(), audit_logs.total);
    
    for entry in &audit_logs.entries {
        println!("ID: {} | Time: {} | Actor: {:?} | Action: {}", 
            entry.id,
            entry.timestamp,
            entry.actor,
            entry.action
    ...
    loop {
        ...
        if !page.has_more || total_shown >= 15 { // Show max 3 pages
            break;
        }
